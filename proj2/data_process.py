...
def load_data(file_dir):
	x_train = pd.read_csv(file_dir+"/X_train.csv", sep=',',header=None, dtype=float, usecols=range(1,1001),skiprows=1).values
	y_train = pd.read_csv(file_dir+"/y_train.csv", sep=',',header=None, dtype=float, usecols=(1,), skiprows=1).values#fillna(value = 0.).values
	x_test = pd.read_csv(file_dir+"/X_test.csv", sep=',',header=None, dtype=float, usecols=range(1,1001),skiprows=1).values #fillna(value = 0.).values
	print(x_train.shape)
	print(y_train.shape)
